...
class Listener(ABC):
    ...
    def is_type_EventManager(self, event) -> None:
        if not issubclass(type(event), EventManager):
            raise f"The [event] is of the wrong type | {self}"
...
class ListenerA(Listener):
    ...
    def update(self, event: EventManager) -> None:
        self.is_type_EventManager(event)
